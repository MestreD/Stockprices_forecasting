...
with features:
    ...
    if company == "None":
        ...
        d = st.date_input("Select a date to compare the actual closing price against the model prediction price for that day (Remember on weekends and holidays the market was closed):", datetime.date(2022,5,16))
        st.spinner(text="In progress...")
        with st.spinner('Wait until the running icon on the top right stops. It might take a couple of minutes...'):
            time.sleep(30)
        d = str(d)
